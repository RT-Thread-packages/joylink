...
#undef DEV_SOFTAP_SSID
#define DEV_SOFTAP_SSID  JOYLINK_SOFTAP_SSID
#endif
...
{
    joylink_dev_get_user_mac((char *)address);
}

static void joylink_clean_wlan_info(void)
{
#define WLAN_CFG_LEN_KEY "wlan_cfg_len"
#define WLAN_CFG_INGO_KEY "wlan_cfg_info"

    /* clean configuration */
    ef_set_env(WLAN_CFG_LEN_KEY, RT_NULL);
    ef_set_env(WLAN_CFG_INGO_KEY, RT_NULL);
...
{
    /* clean wlan information */
    joylink_clean_wlan_info();

    memset((void *)&joy_softap_ram, 0, sizeof(joylinkSoftAPRam));
    ...
    rt_wlan_start_ap((const char*)softap_ssid, RT_NULL);

    while(RT_FALSE == rt_wlan_ap_is_active())
    {
        rt_thread_delay(1);
    }
...
int joylink_softap_udpbroad(int socket_fd)
{
    ...
    joylink_udp_broad_send(socket_fd, (char *)publickey_packet_buf, SOFTAP_BROAD_PUBKEY_PACKETLEN);
...
static void joylink_GeandTx_r2packet(int socket_fd)
{
    ...
    *(pdata + SOFTAP_OFFSET_DATALEN) = datalen;
    ...
    joylink_softap_socket_send(socket_fd, (char *)tx_buf, datalen + SOFTAP_LEN_PACKET_WITHOUTDATA);
